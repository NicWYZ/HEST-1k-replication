...
Stage: R5c, replicate leak generalised to every known same-donor pair (round2_R5_decisions.md directive 2.2).
...
# --------------------------------------------------------- R5c: replicate leak, generalised
# Directive 2.2. The design of the R5 report section 6.1, unchanged, applied to every known
# same-donor pair. For each slide in a pair: hold it out; draw two training sets of IDENTICAL
# size, one from a pool that includes the partner slide and one from a pool that excludes it;
# same test spots, same pipeline, same within-slide metric. The difference is the leak.
#
# PAIRS maps task -> list of same-donor slide groups. IDC's is established in R5
# (10x "FFPE Human Breast using the Entire Sample Area", donorCount: 1, two 5um sections).
# READ's two pairs are HEST's own Patient 1 and Patient 7, which round 3 characterised as
# replicate sections of one specimen rather than merely one patient.
# R5b may add further pairs; they are appended here rather than handled separately.
PAIRS = {
    "IDC":  [["TENX95", "TENX99"]],
    "READ": [["ZEN48", "ZEN49"], ["ZEN36", "ZEN40"]],
}
...
for task, groups in PAIRS.items():
    ...
    n = len(X)
    present = set(samp)
    print(f"[{task}] spots: " + ", ".join(f"{s}={int((samp==s).sum())}" for s in sorted(present)),
          flush=True)

    for grp in groups:
        assert set(grp) <= present, (task, grp, sorted(present))
        for test_slide in grp:
            partner = [s for s in grp if s != test_slide]
            outside = [s for s in present if s not in grp]
            if not outside:
                print(f"  [{task}] {test_slide}: no slides outside the pair, skipped", flush=True)
                continue
            te = samp == test_slide
            pool_with = np.flatnonzero(np.isin(samp, partner + outside))
            pool_without = np.flatnonzero(np.isin(samp, outside))
            # identical training size in both arms: whatever the smaller (no-partner) pool allows
            N = int(min(len(pool_with), len(pool_without)))
            for rep in range(N_REPEATS):
                for arm, pool in (("with_replicate", pool_with),
                                  ("without_replicate", pool_without)):
                    rng = np.random.default_rng(
                        SEED + 7717 * rep + zlib.crc32((task + test_slide + arm).encode()) % 100000)
                    sel = np.zeros(n, bool)
                    sel[rng.choice(pool, size=N, replace=False)] = True
                    assert not (sel & te).any(), "training set overlaps the test slide"
                    record(task, arm, test_slide, rep, None,
                           fit(X[sel], X[te], Y[sel]), Y, samp, te, sel, genes)
            print(f"  [{task}] test={test_slide} partner={partner} outside={sorted(outside)} "
                  f"n_train={N} n_test={int(te.sum())}", flush=True)

d_ = pd.DataFrame(rows)
d_.to_csv(f"{OUT}/replicate_leak_v2__{enc}.csv", index=False)

# per-gene, for R7 (directive 2.2). Explicit schema, per the rule the R3 crash established.
PG = pa.schema([("task", pa.string()), ("design", pa.string()), ("fold", pa.string()),
                ("repeat", pa.int32()), ("grid", pa.int32()), ("slide", pa.string()),
                ("gene", pa.string()), ("pearson", pa.float32()), ("encoder", pa.string())])
pg = pd.DataFrame(pergene, columns=["task", "design", "fold", "repeat", "grid",
                                    "slide", "gene", "pearson"])
pg["encoder"] = enc
pg["fold"] = pg["fold"].astype(str)
pg["repeat"] = pg["repeat"].astype("int32")
pg["grid"] = pg["grid"].astype("Int32")
pg["pearson"] = pg["pearson"].astype("float32")
pq.write_table(pa.Table.from_pandas(pg[[f.name for f in PG]], schema=PG, preserve_index=False),
               f"{OUT}/pergene_replicate_leak__{enc}.parquet", compression="snappy")

print(f"\nwrote {OUT}/replicate_leak_v2__{enc}.csv ({len(d_)} rows) and "
      f"pergene_replicate_leak__{enc}.parquet ({len(pg)} rows)")
w = d_[d_.design == "with_replicate"].groupby(["task", "fold"]).pearson_within.mean()
o = d_[d_.design == "without_replicate"].groupby(["task", "fold"]).pearson_within.mean()
print("\nreplicate leak (with - without), within-slide Pearson:")
for k in sorted(w.index):
    print(f"  {k[0]:<5} test={k[1]:<8} {w[k]:.4f} - {o[k]:.4f} = {w[k]-o[k]:+.4f}")
for t in sorted({k[0] for k in w.index}):
    ks = [k for k in w.index if k[0] == t]
    print(f"  {t} mean leak over {len(ks)} held-out slides: "
          f"{np.mean([w[k]-o[k] for k in ks]):+.4f}")

with open(f"{OUT}/PROVENANCE__r5c_{enc}.txt", "w") as f:
    f.write(
        f"Round 2, stage R5c - replicate leak generalised, encoder {enc}\n"
        ...
        f"script          : code/scripts/round2_r5c_replicate_leak.py\n"
        f"command_line    : {' '.join(sys.argv)}\n"
        f"pythonhashseed  : {os.environ.get('PYTHONHASHSEED', 'unset')}\n"
        f"pairs           : {PAIRS}\n"
        f"n_repeats/seed  : {N_REPEATS}/{SEED}\n"
        "plan            : round2_R5_decisions.md directive 2.2 / plan phase-5c\n")
