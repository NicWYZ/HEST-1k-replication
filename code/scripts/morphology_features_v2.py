...
CHUNK = 20000
os.makedirs(f"{OUT}/morphology_v2", exist_ok=True)
...
summary = []
for task in tasks:
    out_p = f"{OUT}/morphology_v2/{task}_morph.parquet"
...
if summary:
    ...
    s.to_csv(f"{OUT}/morphology_summary_v2__{'_'.join(tasks)}.csv", index=False)
