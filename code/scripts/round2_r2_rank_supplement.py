...
def disp(adatas, genes_common):
    """Normalised dispersion per gene, the benchmark's own ranking statistic."""
    stacked = pd.concat([a.to_df()[genes_common] for a in adatas])
    sa = sc.AnnData(stacked.astype(np.float32))
    sc.pp.filter_genes(sa, min_cells=0)
    sc.pp.log1p(sa)
    sc.pp.highly_variable_genes(sa)
    return pd.Series(sa.var["dispersions_norm"].values, index=sa.var_names), len(stacked)
...
for task in tasks:
    ...
    for A in parts.values():
        g = np.array(sorted(map(str, A.var_names)))
        common = g if common is None else np.intersect1d(common, g)
    common = [g for g in common if "BLANK" not in g and "Control" not in g]
    missing = [g for g in shipped if g not in set(common)]
    assert not missing, f"{task}: shipped genes absent from the common set: {missing[:5]}"

    d_all, n_all = disp(list(parts.values()), common)
    r_all = d_all.loc[shipped].rank(ascending=False)
    ...
    for sp in sorted(glob.glob(f"{BD}/{task}/splits/test_*.csv")):
        ...
        d_tr, n_tr = disp(train, common)
        r_tr = d_tr.loc[shipped].rank(ascending=False)
        ...
        top50_tr = set(d_tr.nlargest(50).index)
        rows.append(dict(
        ...
        print(f"[{task}] fold {k}: spearman {rho:.4f}, kendall {tau:.4f}, "
              ...
              f"{len(top50_tr & set(shipped))}/50 shipped still in the fold's own top 50",
              flush=True)
