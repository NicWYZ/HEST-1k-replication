...
Writes instrumentation/split_comparison__<encoder>.csv
"""
...
for task in tasks:
    ...
    for sp in shipped:
        ...
        rows.append(dict(encoder=enc, task=task, design="patient", fold=k,
    ...
    # --- A: random spot folds, sizes matched to the shipped folds ---
    rng = np.random.default_rng(SEED)
    for k, sz in enumerate(shipped_sizes):
        idx = rng.permutation(n); te = np.zeros(n, bool); te[idx[:sz]] = True; tr = ~te
        m, ng, a = fit_eval(X[tr], X[te], Y[tr], Y[te])
        rows.append(dict(encoder=enc, task=task, design="random_spot", fold=k,
                         n_train=int(tr.sum()), n_test=int(te.sum()), n_genes=ng,
                         alpha=a, pearson=m, n_test_samples=int(len(set(sid[te])))))
        print(f"  {task} random  fold{k}: r={m:.4f}", flush=True)
    ...
    for k, s in enumerate(sorted(set(sid))):
        ...
        if te.sum() < 20 or tr.sum() < 50:
            continue
        m, ng, a = fit_eval(X[tr], X[te], Y[tr], Y[te])
        rows.append(dict(encoder=enc, task=task, design="leave_one_sample", fold=k,
                         n_train=int(tr.sum()), n_test=int(te.sum()), n_genes=ng,
...
d = pd.DataFrame(rows)
d.to_csv(f"{OUT}/split_comparison__{enc}.csv", index=False)
